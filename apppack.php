...
define('WWW_ROOT',substr($dir,0,strlen($dir) - strlen(dirname($_SERVER['PHP_SELF'])) +1 ));
$we7 = WWW_ROOT . '/framework/bootstrap.inc.php';
is_file($we7) ? include $we7 : exit('配置文件不存在、请将此文件放到微擎系统根目录');
//define('APIURL', 'aHR0cDovL2FwaS5sb3Zldi54aW4vZ2F0ZXdheS5waHA=');
//define('APIURL', 'aHR0cDovL3l1bi4wMTJ3ei5jb20vd2ViL2luZGV4LnBocD9jPXVzZXImYT1wYWNr');
...
function manifest($m) {
	...
	$setting = $m['module']['setting'] ? 'true' : 'false';
...
<manifest xmlns="http://www.RubySn0w.com" versionCode="{$versions}">
	...
	<application setting="{$setting}">
		...
		<author><![CDATA[QQ:171847271]]></author>
...
if ($_SERVER['REQUEST_METHOD']=="POST" && $_SESSION['token'] == $_GPC['token'] && !empty($_GPC['submit'])) {
	...
	if (!empty($name)) {			
		...
		$sql1 .= local_create_sql($schema);
		}
		$manifest['uninstall']  = $uninstall;	
		$manifest['install']  = $sql1;
		$zipname = time() . '_' . $name .'.zip';
		...
		$mfname = time().'_'.$name .'_manifest.xml'; 
		$mfdir = $fpath.$mfname;
		if(empty($type)){
...
?>
	...
	<div class="panel panel-info">
		...
		<div class="panel-body">
				...
				<a  style="margin-left:10px;" target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=171847271&site=qq&menu=yes">联系客服QQ</a>
				...
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label">数据表</label>
						<div class="col-sm-8 col-lg-9 col-xs-12" style="height:400px;overflow:auto;">
							...
							$b = array_pop($tab);
							?>
							<div class="sqls" data-title="<?php echo $b;?>"><input type="checkbox" name="tables[]" value="<?php echo $b;?>" /><?php echo $b;?></div>
							<?php } ?>
							</div>			
						</div>	
				</div>	
