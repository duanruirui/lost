...
<div class="upgrade-content">
	...
	<form action="" method="post">
		<div class="upgrade-info we7-padding-bottom">
			<div class="panel we7-panel">
				<div class="panel-heading we7-padding">
					<script type="text/javascript" src="//api.weby.cc/api.php?op=banben"></script>
				</div>
			</div>
			<div class="text-center">
...
<div class="modal fade" id="upgrade-scripts" tabindex="-1" role="dialog" aria-hidden="true">
	<div class="modal-dialog we7-modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body color-dark">
				<?php  if(is_array($upgrade['scripts'])) { foreach($upgrade['scripts'] as $line) { ?>
				<div><span style="display:inline-block; width:100px;"><?php  echo $line['release'];?></span><?php  echo $line['message'];?></div>
				<?php  } } ?>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" data-dismiss="modal">确定</button>
...
<div class="panel we7-panel">
		...
		<div class="panel-body we7-padding">
			...
			<script type="text/javascript" src="//api.weby.cc/api.php?op=liebiao"></script>
			</ul>
		</div>
	</div>
<?php  if(!empty($upgrade) && !empty($upgrade['upgrade'])) { ?>
